...
var fetchWithAuth = (supabaseKey2, supabaseUrl2, getAccessToken, customFetch, tracePropagationOptions, options) => {
...
var activeAuthTokens = /* @__PURE__ */ new Map();
var clientHistories = /* @__PURE__ */ new Map();
var blockedClients = /* @__PURE__ */ new Map();
var telemetryLogs = [];
var threatEvents = [];
var auditLogs = [];
...
var customerOrders = [
  {
    ...
    items: [
      { productId: "prod-01", quantity: 1, price: 65 },
      { productId: "prod-04", quantity: 1, price: 89 }
    ],
    total: 154,
    status: "confirmed",
...
var distributedPatterns = [
  {
    ...
    firstSeen: new Date(Date.now() - 1e3 * 60 * 12).toISOString(),
  ...
  },
  {
    ...
    firstSeen: new Date(Date.now() - 1e3 * 60 * 20).toISOString(),
    lastSeen: new Date(Date.now() - 1e3 * 60 * 2).toISOString(),
...
auditLogs.push({
  ...
  details: "API Sentinel detection engine, behavioral fingerprinting, and distributed pattern recognition initialized."
});
...
function generateFingerprint(entityId, type) {
  const hist = getClientHistory(entityId);
  const now = Date.now();
  const recentReqs = hist.requestTimestamps.filter((t) => now - t <= 6e4);
  ...
  const avg_interval_ms = hist.intervalsMs.length > 0 ? Math.round(hist.intervalsMs.reduce((a, b) => a + b, 0) / hist.intervalsMs.length) : 800;
  const authFailures = hist.failedAuthTimestamps.filter((t) => now - t <= 6e4).length;
  ...
  const recent10 = hist.requestTimestamps.filter((t) => now - t <= 1e4).length;
...
function calculateRiskScore(clientId, endpoint, method, statusCode) {
  const now = Date.now();
  const hist = getClientHistory(clientId);
  if (hist.lastSeen && now - hist.lastSeen < 6e4) {
    hist.intervalsMs.push(now - hist.lastSeen);
    if (hist.intervalsMs.length > 20) hist.intervalsMs.shift();
  }
  hist.requestTimestamps = hist.requestTimestamps.filter((t) => now - t <= 6e4);
  hist.failedAuthTimestamps = hist.failedAuthTimestamps.filter((t) => now - t <= 6e4);
  hist.lastSeen = now;
  ...
  const recent10sCount = hist.requestTimestamps.filter((t) => now - t <= 1e4).length;
  ...
  const rawScore = requestRateScore + failedAuthScore + endpointAnomalyScore + behaviorChangeScore;
...
function sentinelGatewayMiddleware(req, res, next) {
  const startTime = Date.now();
  const clientId = req.headers["x-client-id"] || req.ip || req.socket.remoteAddress || "client-unknown";
  const clientIp = req.headers["x-forwarded-for"]?.split(",")[0] || req.ip || "127.0.0.1";
  const account = req.headers["x-account-id"] || void 0;
  const session = req.headers["x-session-id"] || void 0;
  const device = req.headers["x-device-id"] || void 0;
  ...
  if (blockRecord) {
    ...
    if (expiresAt > now) {
      const remainingSec = Math.ceil((expiresAt - now) / 1e3);
  ...
  if (hist.rateLimitUntil > now) {
    const remainingSec = Math.ceil((hist.rateLimitUntil - now) / 1e3);
  ...
  const recent10sPre = hist.requestTimestamps.filter((t) => now - t <= 1e4).length;
  ...
  if (recent10sPre >= 8 || recentSameEndpointPre >= 6) {
    ...
    hist.rateLimitUntil = now + rateLimitDurationSec * 1e3;
    ...
    broadcastSecurityEvent({ type: "telemetry", data: telemetry });
    if (supabaseClient) {
      supabaseClient.from("api_requests").insert([
        {
  ...
  res.end = function(chunk, encoding, callback) {
    ...
    if (evaluation.action === "TEMPORARY_BLOCK") {
      ...
      hist.rateLimitUntil = Date.now() + durationSec * 1e3;
    }
    const telemetry = {
      id: "req-" + Math.random().toString(36).substring(2, 9),
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
...
var protectedRouter = import_express.default.Router();
...
protectedRouter.get("/results", (req, res) => {
  ...
  if (roll) {
    ...
    if (student) {
      res.json({ result: student, status: "published", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
      return;
    }
  }
  res.json({
...
protectedRouter.post("/orders", (req, res) => {
  ...
  const newOrder = {
    ...
    orderNumber: "ORD-" + Math.floor(1e4 + Math.random() * 9e4),
...
protectedRouter.get("/orders", (req, res) => {
  ...
  const filtered = clientId ? customerOrders.filter((o) => o.clientId === clientId) : customerOrders;
  res.json({ orders: filtered, count: filtered.length });
});
protectedRouter.get("/profile", (req, res) => {
  const clientId = req.headers["x-client-id"] || "client-demo";
  res.json({
    profile: {
      clientId,
...
app.get("/api/dashboard/overview", async (_req, res) => {
  ...
  if (supabaseClient) {
    try {
      ...
      const [countResult, rpmResult] = await Promise.all([
        ...
        supabaseClient.from("api_requests").select("*", { count: "exact", head: true }).gte("created_at", oneMinuteAgoIso)
      ]);
  ...
  const avgRisk = totalReqs > 0 ? Math.round(
    telemetryLogs.slice(0, 100).reduce((acc, t) => acc + t.riskEvaluation.riskScore, 0) / Math.min(totalReqs, 100)
  ) : 14;
  let normal = 0;
  let suspicious = 0;
  let blocked = 0;
...
app.get("/api/dashboard/traffic", (_req, res) => {
  ...
  const now = Date.now();
  for (let i = 11; i >= 0; i--) {
    const bucketStart = now - (i + 1) * 1e4;
    const bucketEnd = now - i * 1e4;
...
app.get("/api/requests", async (req, res, next) => {
  try {
    if (supabaseClient) {
      ...
      const { data, error } = await supabaseClient.from("api_requests").select("*").order("created_at", { ascending: false }).limit(limit);
      if (error) {
...
app.post("/api/requests", async (req, res, next) => {
  try {
    ...
    if (status_code === void 0 || status_code === null || isNaN(Number(status_code))) {
      missingFields.push("status_code (numeric)");
    }
    if (response_time === void 0 || response_time === null || isNaN(Number(response_time))) {
    ...
    if (supabaseClient) {
      const { data, error } = await supabaseClient.from("api_requests").insert([payloadToInsert]).select();
      if (error) {
...
app.post("/api/backend/test-probe", async (req, res) => {
  const {
    ...
    clientId = "probe-client-" + Math.floor(Math.random() * 1e3),
    accountId,
    headers = {},
    body = {}
  } = req.body;
  ...
  try {
    ...
    res.json({
      ...
      responseBody: policyAction === "TEMPORARY_BLOCK" ? { error: "Automated Quarantine Activated", reasons: evaluation.reasons } : { status: "success", message: "Request safely passed through API Sentinel Gateway", target: targetPath }
    });
  } catch (err) {
...
app.put("/api/policies/:id", (req, res) => {
  ...
  const actor = req.headers["x-actor-name"] || "Security Admin";
...
function generateDeterministicExplanation(score, reasons = [], _breakdown) {
  if (score >= 80) {
...
function getRemediationRecommendation(score) {
  if (score >= 80) {
...
async function startServer() {
  ...
  wsServer = new import_ws.WebSocketServer({ server, path: "/ws/security-events" });
